...
module.exports = {
  ...
  single(key) {
    ...
    if (prefixedKeys.hasOwnProperty(key)) {
      return prefixedKeys[key];
    }

    // Key hasn't been prefixed yet, prefix with Modernizr
    const prefKey = Modernizr.prefixed(key);

    // Windows 7 Firefox has an issue with the implementation of
    // Modernizr.prefixed and is capturing 'false' as the CSS property
    // name instead of the non-prefixed version.
    if (prefKey === false) {
      return key;
    }
  ...
  singleHyphened(key) {
    const str = this.single(key);

    return !str
      ? key
      : str.replace(/([A-Z])/g, (m0, m1) => {
